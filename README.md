...
MCP 客户端 (stdio)
    │  JSON-RPC (tools/call)
    ▼
server/server.py          (mcp 2.0 MCPServer)
    │  NDJSON-over-TCP (127.0.0.1:8765)
    ▼
plugin/ DAUnrealMCP       (C++ 插件，自管 FRunnable accept 循环)
    │  ExecPythonCommandEx (游戏线程, Public scope)
    ▼
UE 编辑器 Python VM
```
...
- `plugin/` — UE C++ 插件（本地 TCP 桥 + 游戏线程执行 + NDJSON）
- `server/server.py` — MCP 服务（stdio），暴露 3 个工具 + TCP 客户端
- `server/selftest.py` — stdio 端到端自测
- `server/test_bridge.py` — 直连插件连通性测试
- `deploy.ps1` — 插件部署脚本
- `PROGRESS.md` — 跨会话进度与 resume 文档（含详细编译 / 排查命令）
