...
- **安全与可控**：`execute_python(dry_run=True)` 静态分析预览危险操作（不真执行）；token 鉴权（`endpoint.json`）防止本机其他进程乱连；每次执行落盘 `history.jsonl` 审计可追溯。
- **视觉反馈**：`screenshot()` 截取编辑器视口返回图片（image content），让 AI「看见」脚本改动后的结果。
...
token 鉴权：插件启动时把 token 写到 `<工程>\Saved\DAUnrealMCP\endpoint.json`，server 通过环境变量 `DAUNREAL_MCP_ENDPOINT` 指向该文件后会在每个请求带 token：

```powershell
$env:DAUNREAL_MCP_ENDPOINT = "C:\...\Saved\DAUnrealMCP\endpoint.json"
```

未设置 `DAUNREAL_MCP_ENDPOINT` 时请求不带 token（向后兼容）；插件在 token 文件写入失败时也会自动禁用鉴权。
...
### 脚本执行与内省
- `execute_python(code, mode="sync", dry_run=False)`：核心直通通道。在 UE 编辑器 Python VM 执行任意 Python 脚本，命名空间跨调用持久化（REPL 风格），自动注入 `da` helper。使用 `print(...)` 返回输出。`mode="async"` 时脚本循环被切成时间预算分片逐帧执行（长任务不冻结编辑器），进度经 MCP `report_progress` 上报，支持客户端取消。`dry_run=True` 时仅做 AST 静态分析报告危险操作（delete/destroy/save/spawn 等），不真执行。
...
- `screenshot(width=1280, height=720)`：截取编辑器视口返回图片（image content），让 AI 直接「看见」当前视口。
